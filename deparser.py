...
def create_parser():
    ...
    parser.add_argument('-s', '--start', default=35000, type=int, help="Нижняя цена поиска")
    ...
    parser.add_argument('-t', '--type', default=1, type=int, help="1 - первичный сбор(по умолчанию) "
...
class Car:
    ...
    def __init__(self, page_content):
        ...
        self.engine_type_id = 7
        self.mileage = 0
        self.mileage_type = 2
        self.transmission_type_id = 5
        self.drive_type_id = 4    # 4 = '-'. Отсутствует у донора
        self.color_id = 4
        self.price = None
        self.currency = 1
        self.comment = None
        ...
        self.city_id = 147
        self.name = "Автобай"
        self.phone_country = 7
        self.phone_code = 901
        self.phone_number = 1290777
        self.status_key = 1
        ...
        self.donor = 1
        self.condition = 0
        self.count_doors = None
        self.capacity = None

        property_patterns = {
            'id': None,
            'user_id': None,
            'mark_id': ParsProperty(r"'cm_mod', \[*'(.*?)\..*?'", (MarkModel, "slug"), {"vw": "volkswagen", "-benz": "", " ": "_"}),
            'model_id': ParsProperty(r"'cm_mod', \[*'.*?\.(.*?)'", (ModelModel, ["slug", "mark_id"]), {" ": "_"}, lower_case=True),
            'generation_id': None,
            ...
            'price': ParsProperty(r'<\w+ class="netto-price">([0-9\s]+)', to_digit=True),
            'currency': None,
            'comment': ParsProperty(r'<div class="g-row"><div class="description-text js-original-description g-col-12">'
            ...
            'dealer_id': None,
            'donor': None,
            # 'condition': ParsProperty(r'Состояние.*?</\w+><*/*\w*>*<\w+ class="g-col-6 u-text-bold">(.+?)</\w+>'),
            'count_doors': ParsProperty(r'Число дверей</\w+><\w+ class="g-col-6 u-text-bold">(.+?)</\w+>', replace_array={r"\d+/": ""}, to_digit=True),
            'capacity': ParsProperty(r'Объем двигателя</\w+><\w+ class="g-col-6 u-text-bold">([0-9\s]+)', to_digit=True)
        }
...
class MarkModel(BaseModel):
    def __init__(self):
        ...
        self.popular = 0
        self.logo = None
        self.is_hidden_filter = 1
        self.show_with_logo = 0
        self.label_ru = '%!%(slug)'


class ModelModel(BaseModel):
    def __init__(self):
        self.table_name = "ab_car_model"
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=30) as executor:
        ...
        for future in futures_list:
            try:
                ...
                img_uid = uuid.uuid4()
                query = f"INSERT INTO ab_post_photo (user_id, post_id, uid, image, position, is_main, " \
        ...
        if is_load:
            query = f"UPDATE ab_post SET is_load_photo=1 WHERE id=%s;"
...
def crawl_search(start_price: int, end_price=9999999, check_new: bool = False):
    ...
    if check_new:
        ...
        link = f"{HOST}%D0%BA%D0%B0%D1%82%D0%B5%D0%B3%D0%BE%D1%80%D0%B8%D1%8F/" \
               f"%D0%B0%D0%B2%D1%82%D0%BE%D0%BC%D0%BE%D0%B1%D0%B8%D0%BB%D1%8C/vhc:car,cnt:de," \
               f"pgn:1,pgs:50,srt:date,sro:desc,frn:2019,prn:{start_price},slt:dealer,pwn:125,dmg:false,vat:1"
    else:
        # Поиск с сортировкой по цене от дешевых к дорогим
        link = f"{HOST}%D0%BA%D0%B0%D1%82%D0%B5%D0%B3%D0%BE%D1%80%D0%B8%D1%8F/" \
               f"%D0%B0%D0%B2%D1%82%D0%BE%D0%BC%D0%BE%D0%B1%D0%B8%D0%BB%D1%8C/vhc:car,cnt:de," \
               f"pgn:1,pgs:50,srt:price,sro:asc,frn:2019,prn:{start_price},slt:dealer,pwn:125,dmg:false,vat:1"
...
def crawl_bull(bull_id):
    ...
    """
    cursor = DB.cursor()
    cursor.execute(f"SELECT id FROM ab_post WHERE donor_id=%s LIMIT 1", (bull_id,))
    row = cursor.fetchone()
    if row:
        (value,) = row
    ...
    link_pattern = f"{HOST}vip/0/pg:vipcar/%s.html"
    ...
    if r.status_code == 200:
        ...
        qa = ['%s' if v is not None else 'NULL' for v in values]
        query = f"INSERT INTO ab_post ({', '.join([f'`{k}`' for k in keys])}) VALUES ({', '.join(qa)})"
        out = []
        for v in values:
            if v is not None:
                out.append(v)
        ...
        if car.price:
            query = f"INSERT INTO ab_price (price, post_id, currency) VALUES (%s, %s, %s)"
            cursor.execute(query, (car.price, post_id, car.currency))
...
def check_bull(bull_id):
    ...
    """
    link_pattern = f"{HOST}ajax/vehiclePreview/vhc:car/%s"  # меньше данных подходит для проверки актуальности
    car_link = link_pattern % bull_id
    r = requests.get(car_link, timeout=TIMEOUT, headers=UA, proxies=PROXIES)
    if r.status_code == 403:
        print(f"\n Нас забанили! \n")
...
if __name__ == '__main__':
    ...
    if namespace.type == 1:     # первичный сбор: deparser.py -t 1 -s 35000 -e 50000 -p 127.0.0.1:9988
    ...
    if namespace.type == 2:     # добавление новых объявлений: deparser.py -t 2 -p 127.0.0.1:9988
        start_price = namespace.start
        while start_price < namespace.end:
            print()
            print("+++++++++++++++++++++++++++++++++++++++++++++++++++++++")
    ...
    if namespace.type == 3: # проверка объявлений из базы на удаление у донора: deparser.py -t 3 -p user:pass@@host:port
        del_count = 0
        cursor = DB.cursor()
        try:
            rows = [1]
            ...
            while len(rows) > 0:
                err_cnt = 0
                to_del = []
                cursor.execute('SELECT id, donor_id FROM ab_post WHERE id > %s ORDER BY id ASC LIMIT 300', (post_id, ))
                ...
                if len(to_del) > 0:
                    for p in to_del:  # удаление фото с сервера
                        cursor.execute('SELECT id, image FROM ab_post_photo WHERE post_id = %s', p)
                    ...
                    cursor.executemany('DELETE FROM ab_post WHERE id = %s', to_del)
                    cursor.executemany('DELETE FROM ab_price WHERE post_id = %s', to_del)
                    cursor.executemany('DELETE FROM ab_post_photo WHERE post_id = %s', to_del)
                    DB.commit()
