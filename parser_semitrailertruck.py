...
def create_parser():
    parser = argparse.ArgumentParser(
        prog='parser_semitrailertruck.py',
    )
    subparsers = parser.add_subparsers(dest='command',
                                       title='Возможные аргументы',
                                       description='%(prog)s [options]')
    parser.add_argument('-s', '--start', default=50000, type=int, help="Нижняя цена поиска")
...
def crawl_search(start_price: int, end_price=9999999, check_new: bool = False):
    ...
    if check_new:
        ...
        link = f"{HOST}%D0%BA%D0%B0%D1%82%D0%B5%D0%B3%D0%BE%D1%80%D0%B8%D1%8F/%D1%81%D0%B5%D0%B4%D0%B5%D0%BB%D1%8C%D0%BD%D1%8B%D0%B9-%D1%82%D1%8F%D0%B3%D0%B0%D1%87/" \
               f"vhc:semitrailertruck,pgn:1,pgs:50,srt:date,sro:desc,frn:2022,prn:{start_price},ful:diesel,emc:euro5,dmg:false,vat:1"
    else:
        # Поиск с сортировкой по цене от дешевых к дорогим
        link = f"{HOST}%D0%BA%D0%B0%D1%82%D0%B5%D0%B3%D0%BE%D1%80%D0%B8%D1%8F/%D1%81%D0%B5%D0%B4%D0%B5%D0%BB%D1%8C%D0%BD%D1%8B%D0%B9-%D1%82%D1%8F%D0%B3%D0%B0%D1%87/" \
               f"vhc:semitrailertruck,pgn:1,pgs:50,srt:date,sro:asc,frn:2022,prn:{start_price},ful:diesel,emc:euro5,dmg:false,vat:1"
...
def crawl_bull(bull_id, truck_marks_list):
    ...
    link_pattern = f"{HOST}vip/0/pg:vipsemitrailertruck/%s.html"
...
def check_bull(bull_id):
    ...
    """
    link_pattern = f"{HOST}ajax/vehiclePreview/vhc:semitrailertruck/%s"  # меньше данных подходит для проверки актуальности
    car_link = link_pattern % bull_id
    r = requests.get(car_link, timeout=TIMEOUT, headers=UA, proxies=PROXIES)
    if r.status_code == 403:
        print(f"\n Нас забанили! \n")
...
if __name__ == '__main__':
    ...
    if namespace.type == 1:     # первичный сбор: parser_semitrailertruck.py -t 1 -s 50000 -e 50000 -p 127.0.0.1:9988
    ...
    if namespace.type == 2:     # добавление новых объявлений: parser_semitrailertruck.py -t 2 -p 127.0.0.1:9988
        start_price = namespace.start
        while start_price < namespace.end:
            print()
            print("+++++++++++++++++++++++++++++++++++++++++++++++++++++++")
    ...
    if namespace.type == 3:  # проверка объявлений у донора на удаление: parser_semitrailertruck.py -t 3 -p user:pass@@host:port
        del_count = 0
        cursor = DB.cursor()
        try:
            rows = [1]
